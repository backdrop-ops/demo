...
These instructions explain the Configuration Management workflow.
...
**These instructions are solely for people with access to the live server who
have been asked to deploy recent config changes.**

1. On Backdrop's live server, perform a `git pull` to bring new config files
   into the `staging` directory there.

2. Run update.php to apply any necesary databse updates (this may also update
   config.)

3. Visit the config sync page at `/admin/config/development/configuration` and
   import the recent changes from the staging directory.

4. On Backdrop's live server, perform a `git checkout` on the staging directory
   to restore files that have been deleted from the sync.
